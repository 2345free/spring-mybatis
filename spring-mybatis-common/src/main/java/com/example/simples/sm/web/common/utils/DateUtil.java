...
public class DateUtil {
    ...
    public static String sConvertDate(String strSource, String fromDateFormat, String toDateFormat) {
        ...
        Date date = null;
        if (StringUtil.sGetNullTrimSpace(strSource).trim().equals("")) {
            return "";
        }
     ...
     * 把时间转换成UTC时间格式
     */
    public static String getUTCDate(String d, String format) {
        String rstr = null;
        try {
            SimpleDateFormat foo = new SimpleDateFormat(format);
            foo.parse(d);
...
//			Calendar c = GregorianCalendar.getInstance();
            ...
            nc.setTimeZone(tz);
            Calendar cl = foo.getCalendar();
            int zoneOffset = cl.get(Calendar.ZONE_OFFSET);
            int dstOffset = cl.get(Calendar.DST_OFFSET);
            cl.add(Calendar.MILLISECOND, -(zoneOffset + dstOffset));

            rstr = foo.format(cl.getTime());
            rstr = rstr.replace(" ", "T");
//			Date ud = new Date(cl.getTimeInMillis());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return rstr;
    ...
    public static void main(String[] args) {
        try {
            System.out.println(DateUtil.getUTCDate("2017-1-11 0:30:00", "yyyy-MM-dd HH:mm:ss"));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
